...
{
    public class YearsWithMoreBooks
    {
        private static readonly Guid _userId = Guid.NewGuid();

        [Fact]
        public async Task YearsWithMoreBooks_Success()
        {
            // Arrange
            var service = Service();
            ...
            // Act
            var list = await service.YearsWithMoreBooks(_userId, queryParam);

            // Assert
            Assert.NotNull(list);
            Assert.Equal(2, list.Count);
            Assert.Equal("2021", list[0].Key);
            Assert.Equal(2, list[0].Count);
            Assert.Equal("2020", list[1].Key);
            Assert.Equal(1, list[1].Count);
        }

        [Fact]
        public async Task YearsWithMoreBooks_WhenUserDoesntHaveBooks()
        {
            // Arrange
            var service = Service();
            ...
            // Act
            var list = await service.YearsWithMoreBooks(userWithoutBooksId, queryParam);
        ...
        private static IStatisticsService Service()
        {
            ...
            builder.UseInMemoryDatabase("StatisticsService.YearsWithMoreBooks");
