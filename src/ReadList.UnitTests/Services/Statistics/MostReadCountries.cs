...
{
    public class MostReadCountries
    {
        private static readonly Guid _userId = Guid.NewGuid();

        [Fact]
        public async Task MostReadCountries_Success()
        {
            // Arrange
            var service = Service();
            ...
            // Act
            var list = await service.MostReadCountries(_userId, queryParam);

            // Assert
            Assert.NotNull(list);
            Assert.Equal(2, list.Count);
            Assert.Equal("Estados Unidos", list[0].Key);
            Assert.Equal(2, list[0].Count);
            Assert.Equal("Brasil", list[1].Key);
            Assert.Equal(1, list[1].Count);
        }

        [Fact]
        public async Task MostReadCountries_WhenUserDoesntHaveBooks()
        {
            // Arrange
            var service = Service();
            ...
            // Act
            var list = await service.MostReadCountries(userWithoutBooksId, queryParam);
        ...
        private static IStatisticsService Service()
        {
            ...
            builder.UseInMemoryDatabase("StatisticsService.MostReadCountries");
